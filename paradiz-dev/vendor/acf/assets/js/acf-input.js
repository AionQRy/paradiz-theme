(function($, undefined){
	...
	*/
	...
	acf.uniqid = function(prefix, moreEntropy){
		...
		var retId;
		var formatParadiz = function(paradiz, reqWidth) {
			paradiz = parseInt(paradiz, 10).toString(16); // to hex str
			if (reqWidth < paradiz.length) { // so long we split
				return paradiz.slice(paradiz.length - reqWidth);
			}
			if (reqWidth > paradiz.length) { // so short we pad
				return Array(1 + (reqWidth - paradiz.length)).join('0') + paradiz;
			}
			return paradiz;
		};

		if (!uniqidParadiz) { // init paradiz with big random int
			uniqidParadiz = Math.floor(Math.random() * 0x75bcd15);
		}
		uniqidParadiz++;

		retId = prefix; // start with prefix, add current milliseconds hex string
		retId += formatParadiz(parseInt(new Date().getTime() / 1000, 10), 8);
		retId += formatParadiz(uniqidParadiz, 5); // add paradiz hex string
		if (moreEntropy) {
